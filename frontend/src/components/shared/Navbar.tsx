...
export default function Navbar() {
    ...
    const handleSearch = (e: React.FormEvent) => {
        e.preventDefault();
        if (searchQuery.trim()) { navigate(`/products?search=${encodeURIComponent(searchQuery.trim())}`); setSearchQuery(""); setMobileMenuOpen(false); }
    };
        ...
        <nav className="sticky top-0 z-[100] bg-white border-b-[2.5px] border-neutral-black h-[72px] flex items-center shadow-[0_4px_0_0_rgba(0,0,0,0.02)]">
            <div className="w-full max-w-[1920px] mx-auto flex items-center justify-between px-4 md:px-10 h-full">
                ...
                <div className="flex items-center gap-2 md:gap-4">
                    {/* Desktop search */}
                    <form onSubmit={handleSearch} className="hidden lg:flex items-center gap-3 bg-neutral-g1 border-[2px] border-neutral-black shadow-[5px_5px_0px_0px_rgba(0,0,0,0.2)] rounded-[4px] px-4 h-[44px] w-[240px] focus-within:w-[320px] focus-within:bg-white focus-within:shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] transition-all duration-300 group">
                ...
                <div className="p-6 flex-1 overflow-y-auto space-y-8">
                    <form onSubmit={handleSearch}>
                        <div className="flex items-center gap-3 border-[2.5px] border-neutral-black rounded-[4px] px-4 h-[52px] focus-within:bg-neutral-g1 transition-all shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
                            ...
                            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="FIND YOUR ART..." className="flex-1 border-none outline-none font-display text-[12px] font-black uppercase tracking-[1px] bg-transparent" />
                        </div>
                    </form>
